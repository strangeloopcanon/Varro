...
We introduce the first fully automated **continuous daily closed‑loop** for live financial headlines: ingest, structured roll‑out generation, next‑day LLM‑based scoring, and overnight GSPO updates on real noisy market outcomes.  Each 24 h cycle processes ≈30 streaming financial headlines into eight Qwen-3-0.6B roll‑outs per headline, automatically grades them via a robust two‑stage evaluator, and folds the noisy outcome feedback into offline Group Sequence Policy Optimisation (**GSPO**, \cite{Wang2025GSPO}) updates.  Over five days of live trading‑news, this closed‑loop yields an average normalized reward lift from 0.058 → 0.062 (p≈0.018) while KL divergence remains stable—all on a single Apple M4 Max laptop.  We open‑source code, sample data, and figure scripts as a reproducible blueprint for domain‑driven **offline RL**.
